...
int main(void)
{
	...
	for (i = 0; i < 4; i++){
		g_au32SourceData[u32Data[i]] = u32Data[i];
		g_au32DestinationData[u32Data[i]] = 0;
   ...
   // {
       ...
       // for(u32DataCount = 0; u32DataCount < TEST_COUNT; u32DataCount++)
        ...
        while(1)
        {
            /* Write to TX register */
						printf("%d\n",g_au32SourceData[u32Data[i]]);
            SPI_WRITE_TX0(SPI0, g_au32SourceData[u32Data[i]]);
            /* Trigger SPI data transfer */
            SPI_TRIGGER(SPI0);
            /* Check SPI0 busy status */
            while(SPI_IS_BUSY(SPI0));
            /* Read received data */
            g_au32DestinationData[u32Data[i]] = SPI_READ_RX0(SPI0);
						printf("%d\n",SPI_READ_RX0(SPI0));
            u32Data[i]++;
            if(u32Data[i] > 0)
                break;
       ...
       // for(u32DataCount = 0; u32DataCount < TEST_COUNT; u32DataCount++)
        //{
					//printf("%d\n",g_au32DestinationData[u32DataCount]);
